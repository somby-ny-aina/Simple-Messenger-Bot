...
const menu = [
    { command: "/generate", description: "Generate an image based on the prompt" },
    { command: "/lyrics", description: "Get lyrics for a song" },
    { command: "/help", description: "Show this help message" },
];

const getMenuResponse = () => {
    return menu.map(item => `${item.command} - ${item.description}`).join('\n');
};
...
const getAnswer = async (text, senderId) => {
    if (text.startsWith('/help')) {
        const menuResponse = getMenuResponse();
        return sendMessage(senderId, { text: `Here are the available commands:\n\n${menuResponse}` }, PAGE_ACCESS_TOKEN);
